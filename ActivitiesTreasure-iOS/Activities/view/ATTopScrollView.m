...
@implementation ATTopScrollView

@synthesize nameArray;
@synthesize scrollViewSelectedChannelID;

+ (ATTopScrollView *)shareInstance {
    static ATTopScrollView *_instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instance=[[self alloc] initWithFrame:CGRectMake(0, IOS7_STATUS_BAR_HEGHT + menuHeight, CONTENTSIZEX, menuHeight)];
    });
    return _instance;
}
...
- (void)initWithNameButtons
{
    ...
    for (int i = 0; i < [self.nameArray count]; i++) {
        ...
        [button setTitleColor:[ATGloble colorFromHexRGB:@"868686"] forState:UIControlStateNormal];
        [button setTitleColor:[ATGloble colorFromHexRGB:@"bb0b15"] forState:UIControlStateSelected];
...
- (void)selectNameButton:(UIButton *)sender
{
    ...
    if (!sender.selected) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            ...
            if (finished) {
                //设置新闻页出现
                [[ATRootScrollView shareInstance] setContentOffset:CGPointMake(BUTTONID*320, 0) animated:YES];
                //赋值滑动列表选择频道ID
